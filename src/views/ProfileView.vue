...
// Thêm hai biến ref để kiểm soát việc hiển thị của hai div
const showUserInfo = ref(true);
const showOrderDetails = ref(false);

const HandleProd =ref(false);
...
onMounted(() => {
  customer.value = authStore.customer;
});
</script>
...
<template>
  <div class="container">
    ...
    <div v-if="showUserInfo" class="info-user">
      <h2>Thông tin cá nhân</h2>
      <p>Họ tên: {{ customer ? customer.name : '...' }}</p>
      <p>Email: {{ customer ? customer.email : '...' }}</p>
      <p>Số điện thoại: {{ customer ? customer.phone : '...' }}</p>
    ...
    <div v-if="showOrderDetails" class="order-details">
      <div class="order-card">
        ...
        <div class="order-item">
          ...
          <div class="status">
            <span v-if="HandleProd" class="canceled">Đang giao hàng</span>
            <span v-else class="canceled">Đã hàng</span>
            <p>Thành tiền: <span class="total-price">108.199 VNĐ</span></p>
            <button v-if="!HandleProd" class="buy-again">Đánh giá</button>
          </div>
        </div>
      </div>
    </div> 
